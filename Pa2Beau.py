...
class Pa2Beau(wx.Frame):
    ...
    def __getPlotData(self, bbModel, dtini, dtfin, pts, do_merge):
        ...
        """
        res = bbModel.xeq(dtini, dtfin, Pa2Beau.CLC_DELTAT, pts, do_merge)
        dtmax = dtfin
        for pt, dtaPt in res:
            for dtaTr in dtaPt:
                for dtaXpo in dtaTr:
    ...
    def __getPlotDataZoom(self, bbModel, dtini, dtfin, pts, do_merge):
        ...
        point, tides = pts
        for t in tides:
            r = bbModel.xeq(dtini, dtfin, Pa2Beau.CLC_DELTAT, [ [point, [t]] ], do_merge)
            res.extend(r)
        dtmax = dtfin
        for pt, dtaPt in res:
            for dtaTr in dtaPt:
...
def createPaBeauApp(*args, **kwargs):
    ...
    #   def OnInit(self):
    frame = Pa2Beau(None, -1, '', *args, **kwargs)
