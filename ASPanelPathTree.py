...
try:
    ...
    pass
LOGGER = logging.getLogger("INRS.ASur.panel.path.tree")
...
class ASPanelPathTree(wx.Panel):
    ...
    def setNode3State(node):
        ...
        if node.HasChildren():
            ...
            ret = 1 if node.IsChecked() else 0
        return ret
    ...
    def fillTree(self, plumes):
        ...
        LOGGER.trace("Plumes:")
        for p in plumesSorted:
            LOGGER.trace("   %s", str(p))
    ...
    def __getAllItems(self, node):
        res = []
        if node.HasChildren():
            if node.GetData():
                res = [ node.GetData() ]
            for child in node.GetChildren():
                res.extend( self.__getAllItems(child))
        else:
            res = [ node.GetData() ]
        return res
    ...
    def getAllItems(self):
        ...
        """
        LOGGER.trace('ASPanelPathTree.getAllItems')
        root = self.tree.GetRootItem()
        return self.__getAllItems(root) if root else []
